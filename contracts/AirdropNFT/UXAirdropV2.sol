...
contract UXAirdropV2 is UXSign {
    ...
    function claimERC20(
        ...
        bytes32 claimERC20SaltCode = keccak256(abi.encodePacked(saltCode, uxerc20token));
        require(verifySignature(transId, bytes32ToHexString(claimERC20SaltCode), tokenAmount, msg.sender, signature), "Signature is invalid");
        require(IERC20(uxerc20token).balanceOf(address(this)) >= tokenAmount,"TOKEN Balance is insufficient!");
        setTransId(transId);
        // send token to user
        ...
        emit ClaimedERC20Success(msg.sender, msg.sender, uxerc20token, tokenAmount, transId);
    }
    ...
    function claimERC20ToAddress(
        ...
        emit ClaimedERC20Success(msg.sender, toAddress, uxerc20token, tokenAmount, transId);
    }
    ...
    function bytes32ToHexString(bytes32 data) internal view returns (string memory) {
